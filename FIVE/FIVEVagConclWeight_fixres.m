function [Y] = FIVEVagConclWeight_fixres(U, VE, R, X, P)
%FIVEVagConcl: Calculate the weight of the rule match from observation
%
...
%
    ...
    % Modified by David Vincze, 23.04.09
    ...
    rd = FIVERuleDist_fixres(U, VE, R, X); % calculate the distances of the observation from the rule antecedents
...
%     %                vrconc=[vrconc,FIVEVagDist(U(m,:),VE(m,:),U(m,1),R(i,m))];
%                     vrconc=[vrconc,FIVEVagDist(U(m,:),VE(m,:),U(m,1),1)];
...
%             if m==nu    % the consequent universe also has a vague environment
%                 % vague distance from the first (smallest) element of the conclusion universe
%     %            vrconc(i)=FIVEVagDist(U(m,:),VE(m,:),U(m,1),R(i,m));
%                 vrconc(i)=FIVEVagDist(U(m,:),VE(m,:),U(m,1),1);
%             else    % the rule consequences are singletons
%     %            vrconc(i)=R(i,m);
%                 vrconc(i)=1;
%             end
...
%             vagc=vagc./ws;  % The valid interpolated conclusion
            ...
            for i = 1:n
                vagc(i) = wi(i) ./ ws; % The valid interpolated conclusion
            end
