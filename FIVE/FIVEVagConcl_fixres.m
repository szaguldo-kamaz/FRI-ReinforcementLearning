function [Y] = FIVEVagConcl_fixres(U, VE, R, X, P)
...
%
    ...
    if nargin == 4% P is not given
        P = mr - 1; % the number of the antecedents
    end
    ...
    if sum(rd == 0) > 0 % at least one rule antecedent had exactly hit
        ...
        for i = 1:nr % for all the rules

            if rd(i) == 0

                if mr == nu % the consequent universe also has a vague environment
                    % vague distance from the first (smallest) element of the conclusion universe
                    vrconc = [vrconc, FIVEVagDist_fixres(U(mr, :), 1, mu, VE(mr, :), U(mr, 1), R(i, mr))];
                else % the rule consequences are singletons
                    vrconc = [vrconc, R(i, mr)];
                end
        ...
        for i = 1:nr % for all the rules
            ...
            if mr == nu % the consequent universe also has a vague environment
                % vague distance from the first (smallest) element of the conclusion universe
                vrconc(i) = FIVEVagDist_fixres(U(mr, :), 1, mu, VE(mr, :), U(mr, 1), R(i, mr));
            else % the rule consequences are singletons
                vrconc(i) = R(i, mr);
            end
    ...
    if isnan(vagc)
        ...
        if mr == nu % the consequent universe also has a vague environment
            Y = FIVEValVag_fixres(U(mr, :), VE(mr, :), vagc); % The valid conclusion
        else % the rule consequences are singletons
